...
const Steganography: React.FunctionComponent = (props) => {
    ...
    const hide = async () => {
        if (path.extname(image).toLowerCase() === ".png") {
            ...
            // @ts-ignore
            FS.writeFile("hide.jpg", new Uint8Array(arrayBuffer))
            if (embedFile) {
                const fileBuffer = await fetch(embedFile).then((r) => r.arrayBuffer())
                // @ts-ignore
                FS.writeFile(filename, new Uint8Array(fileBuffer))
            } else {
                // @ts-ignore
    ...
    const retrieve = async () => {
        if (path.extname(image).toLowerCase() === ".png") {
            ...
            const arrayBuffer = await fetch(image).then((r) => r.arrayBuffer())
            // @ts-ignore
            FS.writeFile("seek.jpg", new Uint8Array(arrayBuffer))
            try {
                // @ts-ignore
                Module.ccall("seek", "number", ["string", "string", "string"], ["seek.jpg", filename, encryptionKey], null)
                // @ts-ignore
